...
namespace Microsoft.Reporting.WinForms
{
	internal class DataSetExtensionConnection : IProcessingDataExtensionConnection
	{
		...
		// callers must re-assert which data sources apply immediately before actually opening a
		// connection for a given dataset (see RuntimeDataSource.OpenConnection).
		public void EnsureCorrectDataSources(bool inSubreport)
		{
			m_dataSources = inSubreport ? (m_lastSubreportDataSources ?? m_dataSources) : m_rootDataSources;
		}
