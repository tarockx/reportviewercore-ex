...
namespace Microsoft.ReportingServices.Rendering.HPBProcessing
{
    internal sealed class ReportSection
    {
        ...
        internal double NextPage(RPLWriter rplWriter, int pageNumber, int totalPages, double top, double availableHeight, ReportSection nextSection, bool isFirstSectionOnPage)
        {
            ...
            bool shouldRenderHeader = true;
            bool shouldRenderFooter = true;
            //switch (pageNumber)
            //{
            //	case 1:
            ...
            bool lastSection = nextSection == null;
            if (!lastSection)
            {
            ...
            PageSection currentSectionHeader = m_reportSection.Page.PageHeader;
            PageSection currentSectionFooter = m_reportSection.Page.PageFooter;
            bool printHeaderBetweenSections = currentSectionHeader?.PrintBetweenSections ?? false;
            bool printFooterBetweenSections = currentSectionFooter?.PrintBetweenSections ?? false;

            //var pageName = m_reportBody.DD(m_pageContext, m_topEdge, m_topEdge + availableHeight);
            var tempCtx = new PageContext(m_pageContext);
            //m_reportBody.DetermineVerticalSizePublic(tempCtx, m_topEdge, m_topEdge + availableHeight);
            bool anyAncestorHasKT_Discard = false;
            m_reportBody.CalculateVertical(m_pageContext, m_topEdge, m_topEdge + availableHeight, null, new List<PageItem>(), ref anyAncestorHasKT_Discard, hasUnpinnedAncestors: false);
            var pageName = tempCtx.PageName;
            if (currentSectionHeader != null && pageName?.Contains("EprNoHeader") == true)
            {
                //hasHeaderToRender = false;
                //availableHeight += m_sectionPageSettings.HeaderHeight;
                //headerHeight = m_sectionPageSettings.HeaderHeight;
                shouldRenderHeader = false;
            }
            if (currentSectionFooter != null && pageName?.Contains("EprNoFooter") == true)
            {
                //hasFooterToRender = false;
                //availableHeight += m_sectionPageSettings.FooterHeight;
                shouldRenderFooter = false;
            }

            m_needsHeaderHeight = (isFirstSectionOnPage ? (currentSectionHeader != null) : printHeaderBetweenSections) && shouldRenderHeader;
            if (m_needsHeaderHeight)
            {
                headerHeight += m_sectionPageSettings.HeaderHeight;
                availableHeight -= headerHeight;
            }
            if (currentSectionFooter != null && shouldRenderFooter)
            {
                availableHeight -= m_sectionPageSettings.FooterHeight;
            }
            bool hasHeaderToRender = false;
            bool hasFooterToRender = false;
            ...
            bool notCountingPages = m_pageContext.PropertyCacheState != PageContext.CacheState.CountPages;
            m_pageContext.EvaluatePageHeaderFooter = false;
            if (needsReportItemsOnPage && notCountingPages)
            {
                if (shouldRenderHeader && ((printHeaderBetweenSections && !isFirstSectionOnPage) || HasHeaderOnPage(pageNumber, totalPages)))
                {
                    hasHeaderToRender = true;
                }
                if (shouldRenderFooter && (HasFooterOnPage(pageNumber, totalPages) || (printFooterBetweenSections && !lastSection)))
                {
                    hasFooterToRender = true;
                }
                if (hasHeaderToRender || hasFooterToRender)
                {
            ...
            double remainingHeight = availableHeight - m_columnHeight;
            if (Done && !lastSection && currentSectionFooter != null && !printFooterBetweenSections)
            {
                remainingHeight += m_sectionPageSettings.FooterHeight;
            }
            bool flag7 = false;
            if (remainingHeight < num || lastSection)
            {
                m_columnHeight = availableHeight;
                flag7 = true;
                headerHeight += (shouldRenderFooter ? m_sectionPageSettings.FooterHeight : 0);
            }
            else if (printFooterBetweenSections)
            {
                headerHeight += (shouldRenderFooter ? m_sectionPageSettings.FooterHeight : 0);
            }
            m_itemPageSizes.Height = headerHeight + m_columnHeight;
            m_itemPageSizes.Top = top;
            WriteColumns(rplWriter);
            m_columns = null;
            if (Done && lastSection && totalPages == 0)
            {
                totalPages = pageNumber;
            }
            if (notCountingPages)
            {
                hasHeaderToRender = shouldRenderHeader && ((!isFirstSectionOnPage) ? printHeaderBetweenSections : HasHeaderOnPage(pageNumber, totalPages));
                hasFooterToRender = shouldRenderFooter && ((!flag7) ? printFooterBetweenSections : HasFooterOnPage(pageNumber, totalPages));
            }
            if (hasHeaderToRender || hasFooterToRender)
            {
                ...
                if (hasHeaderToRender && !IsHeaderUnknown(isFirstSectionOnPage, pageNumber, totalPages))
                {
                ...
                if (rplWriter != null && (hasHeaderToRender || hasFooterToRender))
                {
